...
Dom.getElementsByTagName = function(object, tag, filter, clone) {
    ...
    if (!object || !object.childNodes) return result;

    if (tag != null && !(tag instanceof RegExp)) tag = new RegExp("^\\s*" + Dom.escapeRegExp(tag) + "\\s*$", "i");

    if (filter != null && !(filter instanceof RegExp)) filter = new RegExp("^\\s*" + Dom.escapeRegExp(filter) + "\\s*$", "i");

    for (loop = 0; loop < object.childNodes.length; loop++) {

        entry = object.childNodes[loop];

        if (tag != null) tag = new RegExp(tag);

        if (filter != null) filter = new RegExp(filter);

        if ((tag == null || tag.test(entry.nodeName)) && (filter == null || (entry.getAttribute && filter.test(entry.getAttribute("id"))))) {
...
Dom.getParentElementByTagName = function(object, tag, filter, clone) {
    ...
    while (object != null && object.parentNode && object.parentNode != object) {
        ...
        if (object != null && (tag == null || tag.test(object.nodeName)) && (filter == null || (object.getAttribute && filter.test(object.getAttribute("id"))))) return clone ? object.cloneNode(true) : object;
    }
...
/**
 *  Rueckgabe der HTML-Struktur fuer das uebergebene Objekt.
 *  @param  object auszulesendes Objekt
 *  @return die HTML-Struktur fuer das uebergebene Objekt
 */
                ...
                if (rule)  {
                    ...
                    if (rule.selectorText.toLowerCase() == name) {
                        ...
                        if (remove) {
                            ...
                            if (style.cssRules) style.deleteRule(index);
 ...
 */
Dom.addCssRule = function(name) {

    if (document.styleSheets && !getCSSRule(name)) {                        

        if (document.styleSheets[0].addRule) document.styleSheets[0].addRule(name, null, 0);
        else document.styleSheets[0].insertRule(name + ' { }', 0);
    }                           

    return getCSSRule(name);
};
